...
| World origin | axes **+X East / +Y North / +Z up [DATA]**; **`(0,0,0)` = SW (min-E, min-N) corner of landblock `(0,0)` [DERIVED]** (from axes + intra-block `[0,192)` clamp + id encoding). A block `(LbX,LbY)` spans AC-m `X∈[LbX·192,(LbX+1)·192]`, `Y∈[LbY·192,…]`; local coords `[0,192)` from that SW corner. Stored **landblock-LOCAL**: the `objcell_id` field is [DATA]; `+ intra-cell frame / never-global` is [REF-IMPL: ACE wire `Position`]. Render frame floats around the viewer's block (`calc_frame`; setter `calc_draw_order`). **Issue #3.** | `[acclient: 0x504F90 axes; 0x505460+0x505C70 float origin; 0x453180 objcell_id]`; see [`notes/client-landblock-load-radius-findings.md`](../../docs/migration/notes/client-landblock-load-radius-findings.md) |
