"""_doorfix_rebuild.py - rebuild SM_Setup_020005DA (the academy cave door)
in place from the placement-fixed OBJ, preserving material bindings.

The committed mesh was built from the OLD exporter that only looked for the
Resting (0x65) placement frame. Setup 0x020005DA ships ONLY a Default (0x00)
frame, so the old export applied no per-part transform and all 5 parts
collapsed onto the setup origin. The fixed exporter falls back to Default(0),
placing the parts (mirrored jambs + raised lintel). Topology is identical
(same parts, tris, slots) - only vertex positions move.

Mechanism mirrors _propfix_rebuild.py (proven on the 430-cell + 135-prop
fixes): capture slot bindings, delete+recreate from the current OBJ,
re-stamp slots preserving the captured material per slot_name, verify.
Does NOT load the level or save the umap; actors resolve the asset path on
next level load.

Writes pipeline/renders/_doorfix_rebuild.txt.
"""
import sys, math
...
OUT = REPO / "pipeline" / "renders" / "_doorfix_rebuild.txt"
DOOR_HEX = "020005DA"
...
lines = []
def out(m):
    lines.append(str(m)); unreal.log(f"[doorfix] {m}")
def flush():
    OUT.write_text("\n".join(lines) + "\n")
...
asset_name = f"SM_Setup_{DOOR_HEX}"
asset_path = f"{SETUPS_PACKAGE}/{asset_name}"
obj_path = OBJ_DIR / f"setup_{DOOR_HEX}.obj"

# ----- PREFLIGHT (no mutation) -------------------------------------------
if not EAL.does_asset_exist(asset_path):
    out(f"ABORT: asset missing {asset_path}"); out("RESULT: NEEDS-REVIEW"); out("DONE"); flush(); sys.exit(0)
if not obj_path.exists():
    out(f"ABORT: OBJ missing {obj_path}"); out("RESULT: NEEDS-REVIEW"); out("DONE"); flush(); sys.exit(0)
...
out(f"door {DOOR_HEX}: old_tris={old_tris} obj_tris={o_tris} old_verts={old_verts}")
out(f"  old_slots={old_slots_ordered}")
out(f"  obj_slots={sorted(o_slots)}")
...
if anomalies:
    ...
    out("ABORT: preflight anomalies - asset NOT modified."); out("RESULT: NEEDS-REVIEW"); out("DONE"); flush(); sys.exit(0)
...
try:
    ...
    new_slots = build_desc_and_save(obj_mesh, sm, asset_path, prev_mats)
    ...
    out("=" * 50)
    out(f"REBUILT door: tris {old_tris}->{new_tris}, verts {old_verts}->{new_verts}")
    out(f"  new_bounds={tuple(round(x,1) for x in nb)}")
    out(f"  bounds extent grew: old_ext={tuple(round(old_bounds[k],1) for k in (3,4,5))} new_ext={tuple(round(nb[k],1) for k in (3,4,5))}")
    if problems:
        out(f"VERIFY-FAIL: {'; '.join(problems)}")
        out("RESULT: NEEDS-REVIEW")
    else:
        out("RESULT: CLEAN")
