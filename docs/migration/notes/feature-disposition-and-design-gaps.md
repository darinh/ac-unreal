...
| Lighting model | per-vertex Gouraud computed live; per-Setup `LightInfo`; per-cell ambient; no baked vertex color [DATA/REF-IMPL] methodology §5, glossary | **IMPROVE** (per-pixel dynamic) | GAP: only the **indoor unlit-emissive expedient** is decided (ADR-0007). Outdoor/day-night lighting model is **undecided** → needs ADR. |
