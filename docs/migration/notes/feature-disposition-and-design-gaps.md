...
| Sky / fog / sun-moon / day-night (`RegionDesc 0x13`) | global sky/sun/moon/fog/ambient + "lighting of day" [DATA] methodology §2/§5; glossary | **MIRROR** (data-driven) + **IMPROVE** (SkyAtmosphere/volumetrics) | GAP closed (2026-06-02, issue #2): extracted → `pipeline/dat-extract/samples/region_dereth.json` (`acdat export-region`). Author sky/fog **from those values** — still do not invent. Time-of-day drive system unbuilt. |
...
| Day-night / weather | `RegionDesc.SkyDesc` "lighting of day"; weather [DATA] methodology; [DOC] README §XV | **MIRROR** (data-driven) + **IMPROVE** (volumetrics) | GAP: extraction **DONE** (2026-06-02 → `samples/region_dereth.json`); the time-of-day drive system is still unbuilt. |
...
3. ✅ `RegionDesc.SkyDesc` contents — **extracted 2026-06-02** (`acdat export-region` → `pipeline/dat-extract/samples/region_dereth.json`): fog (`Min/MaxWorldFog` + `WorldFogColor`), ambient/directional colours, sky objects, day-night period (`DayLength=7620s` / `DaysPerYear=360`). Angle units (`DirHeading`/`DirPitch`) still `[VERIFY]`.
...
| Weather (beyond day-night) | weather **state data** exists in `RegionDesc.SkyDesc` [DATA: `samples/region_dereth.json` — 20 named `DayGroup`s w/ `ChanceOfOccur` weights]; whether the retail **client's** runtime weather-selection + precipitation system consumes them that way is **`[VERIFY]`** (not yet traced) | **MIRROR** (data-driven) + **IMPROVE** (modern FX) | Partially unblocked 2026-06-02 (issue #2): `export-region` extracted the sky-state presets — `SkyDesc.DayGroups` = Sunny/Rainy/Clear/Cloudy, each w/ `ChanceOfOccur` + per-state sky objects (Rainy ~19 incl. `DefaultPESObjectId` particle refs vs Sunny ~7). Remaining `[VERIFY]`: runtime weather-selection + precipitation-FX wiring (which PES/GfxObj → rain/snow). |
