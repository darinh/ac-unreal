...
| 3 | **Furniture & decorations** — the room's Stab/Setup objects placed + textured. Fixed the `DumpAcademyStatics` coord doubling (median static→cell 4.1 m, 598/644 within 6 m); re-exported setup meshes clean (0/208 NaN); placed 644 actors + bound materials. Runbook: [`runbook/step3-furniture.md`](runbook/step3-furniture.md). | V, III | Props present at correct transform + textured ✓; per-object orientation/identity audit vs answer key pending. | `[PARTIAL]` core done |
...
- **Portal-opening extraction** (cells): `ExportEnvCell` skips `Stippling == NoPos` portal polygons (mirroring ACViewer), so see-through openings reveal the adjacent cell instead of an opaque placeholder face. Proven academy-wide via `audit-portals` (568/568 cells, `NoPos ⊆ Portals`, 0 violations, 0 empty). First room `0x860201AD` rebuilt + **user-accepted**; ADR-0008. Re-import of the other 567 cells is a follow-up. (III)
- Headless render-verification harness, with caveats (XIII).
- **First-room shell coexistence (Step 0/1)** — `0x860201AD` walls+floor+ceiling render correctly *together* via a fixed 6-view sweep (`render_firstroom_sweep.ps1`) that passes `test_renders.py --manifest` (all 6 PASS, no black/magenta/flat, sky_frac ≤0.033 ≪ 0.30). Evidence: `pipeline/renders/firstroom_sweep_20260601_080757/`; method + per-view metrics in [`runbook/step0-identify-first-room.md`](runbook/step0-identify-first-room.md). **User sign-off 2026-06-01** (GitHub issue #1). (II, III, XIII)
...
- **2026-06-01 (Academy-wide NoPos portal-skip rollout — see-through doorways, user sign-off)** Scaled the proven 2-cell portal fix to the whole academy: re-exported all 568 cells with the ADR-0008 `NoPos`-skipping extractor and rebuilt **430** stale `Content/Academy/Cells/SM_8602*.uasset` meshes in place (they were drawing portal/doorway polygons as solid off-white `MI_Color_000000` faces — the "off-white archway between cells"). 245 of the 430 dropped the `MI_Color_000000` sentinel slot. The remaining 138 cells were already correct (the 2 proof cells `01B4`/`01B1` + the earlier 224 rebuild + meshless cells) and were left untouched. **Rebuild is in-place** (`build_static_mesh` preserves per-slot material bindings + saves the asset; the umap and actor→mesh references are NOT touched). **Adversarial review** (rubber-duck) hardened the rollout script before running: `committed < fresh` is a halt-and-report **anomaly** (not silently skipped), and every rebuild is reloaded from disk and gated on `new_tris == fresh_OBJ_tris` + no `surf_*` slot losing its material. Run result **CLEAN**: 430 rebuilt, **0 anomalies, 0 verify failures, 0 errors, 0 missing OBJ**. Safety: git diff is **exactly 430 files, all `Content/Academy/Cells/SM_*.uasset`** (LFS pointer→pointer), umap untouched; first-room coexistence sweep **MANIFEST PASS** (no shell regression); the original off-white doorway view now renders see-through into the deeper rooms (those deeper cells were in this batch). Same mechanism/ADR as the 2-cell proof (`80a92e4`) and the 224-cell rebuild. `audit-portals 0x8602` background invariant unchanged: every skipped polygon is a declared portal, 0 violations.

- **2026-06-01 (Academy-wide cell rebuild — 224 empty-stub cells restored; closes the "re-import the other 567 cells" open item)** Rebuilt **224** `Content/Academy/Cells/SM_8602*.uasset` meshes that were ≤200-byte empty stubs at HEAD (`0c4b026`) — invisible holes in rooms beyond the first — with real geometry + baked unlit material bindings, so **567/568** cells now carry surface materials. Change is **purely additive**: the first-room cells (incl. `SM_860201AD`) are untouched, so the issue-#1 coexistence sign-off does not regress. **Adversarial review** (rubber-duck pass) + binary-asset audit: the 224 rebuilt cells reference **31** distinct `MI_########` materials, **all present and git-tracked → zero dangling references**; the lone no-`MI_` cell `SM_860202D1` uses the tracked `MI_Color_000000` black-sentinel (renders solid warm-grey, not hot-pink/checker). Two new materials `MI_06003F40`/`MI_06003F41` were left **uncommitted** — a binary string scan across all cells/setups/the level found **zero** referencers (dead artifacts). Render evidence (LFS-smudged working tree): a rebuilt cell at UE `(-23000,15000)` renders solid textured stone walls/floor; the `MI_Color` outlier `SM_860202D1` at `(-15000,16000)` renders solid grey; the untouched first room still renders walls+floor+wood-ceiling+green fireplace w/ embers (no regression). Note: `.uasset`/`.umap` are **Git LFS**, so `git cat-file -s HEAD:…` returns the ~132-byte pointer, not the asset — the committed level is the 2.4 MB populated map. (Open: full-academy 6-view manifest sweep is blocked by the known streaming hang; per-room coexistence sweeps remain a follow-up.)

- **2026-06-01 (Step 0/1: first-room shell coexistence PROVEN, user sign-off — issue #1)** Broke the render regression cycle with a fixed-camera "coexistence formula": `pipeline/ue-import/render_firstroom_sweep.ps1` renders 6 pre-declared cameras from the first room's center (UE -3000,1000,250) — `wall_n/e/s/w` + `ceiling` (pitch +75) + `floor` (pitch -75) — into one timestamped dir, and `test_renders.py --manifest` requires ALL 6 to pass absolute checks (mean 15–245, content ≥0.45, magenta ≤0.015, color_std ≥14) + a sky-leak gate (sky_frac ≤0.30). This makes "walls+floor+ceiling coexist" re-runnable and regression-guarded, vs a cherry-picked angle. Result: `MANIFEST PASS`, exit 0, all 6 views PASS (wall_n 46.3/0.927, wall_e 70.8/0.964, wall_s 43.7/0.906, wall_w 54.2/0.947, ceiling 22.0/0.623, floor 43.8/0.936; max sky_frac 0.033). Visual match vs answer key confirmed (tan plaster walls + grey base band, green mossy fireplace w/ embers, blue tile floor, wood-beam ceiling through the `0x...02E2` `NoPos` portal). Untextured furniture statics are Step-3 scope. Evidence dir `pipeline/renders/firstroom_sweep_20260601_080757/`; runbook updated; §3 steps 0+1 promoted to `[PROVEN]`. (Furniture identity/orientation audit remains Step 3.)

- **2026-06-01 (Step 3: prop/furniture UV fix, user sign-off)** Props rendered stretched / as flat "plaster" (the framed map-of-Dereth collapsed to a smear) because the **prop** exporter `ExportSetup` ignored per-corner `PosUVIndices` — the identical bug already fixed in `ExportEnvCell` for cells. **Fixed `ExportSetup`** to emit one `vt` per (vertex,UV) and select per corner via `PosUVIndices` (fallback UV 0), winding preserved; acdat rebuilt clean. The canonical prop OBJs already carried the fix from a prior session, so the visible defect was **stale UE meshes** built from pre-fix OBJs. Rebuilt the **135 stale** `SM_Setup_*` meshes in place from the current OBJs via a dedicated material-preserving script (`_propfix_rebuild.py`: two-phase preflight+mutate, delete+recreate at same path, re-stamp materials by slot_name, verify tris/slots/materials/bounds) — same safe mechanism as the 430-cell portal fix; **65 props with no UV splits (`vt==v`) left untouched** (regression-safe). The old committed bounds were corrupt/placeholder (e.g. `010000A3` = `origin(inf,inf,inf)`, the map = extent `800,800,800`); the rebuild produces correct geometry-derived bounds (verify gate validates new bounds vs OBJ referenced-vertex geometry). Verified: `test_renders.py --manifest` **MANIFEST PASS** (shell intact, `AcademyMap.umap` untouched), and the first-room wall renders now match the answer key — map of Dereth legible + gold-framed, red tapestry's full ornate pattern, bookshelves/fireplace/urns/helmets/candlesticks all correctly textured, no stretching/reversal. Adversarial (rubber-duck) reviewed: metadata-loss risk closed by importer-parity (rebuild matches the original `build_setup_mesh` construction path, plus preserves materials). Scope: 135 `Content/Academy/Setups/*.uasset` + `Program.cs`. (Open: `ExportNpc` has the same unfixed bug — Step 4; the 65 `vt==v` props' uniform-UV-index edge case is an unmeasured follow-up, not a regression.)
- **2026-05-31 (EnvCell↔landblock co-location RESOLVED, user sign-off)** Settled ADR-0009's #1 gating assumption (A1) with a proven, reproducible census: EnvCell world placement is `world=(LbX*192+Fx, LbY*192+Fy, Fz)` with no Z terrain offset [REF-IMPL: ACViewer `PositionExtensions.cs:22-29`, `R_Landblock.cs:51-56`, `Buffer.cs:340-344`, `InstanceBatch.cs:98-105`, `LandDefs.cs:102`; ACE `Position.cs:123`]. Co-location is **type-dependent**: 8/8 sampled building-interior blocks (`Buildings>0`) are fully co-located (**882/882 cells IN footprint**); 3/3 zero-building blocks are mostly **OUT** (2200/2259, into negative-Y). New auditable tooling `acdat find-building-blocks` (1639/5346 blocks have buildings) + `dump-envcell-positions` (single-process full census, validated to reproduce the prior per-cell census exactly); raw artifact `pipeline/dat-extract/samples/envcell_position_census.txt`. Corrected from two earlier mis-conclusions under ruthless multi-model adversarial review (Sonnet 4.6 / GPT-5.3-Codex / MAI-Code / Opus) + two-family re-verification (GPT-5.5 / GPT-5.3-Codex). Durable record: [`notes/envcell-colocation-findings.md`](notes/envcell-colocation-findings.md); cross-linked from ADR-0009 A1, ADR-0010 A2, ADR-0013 A2. **Still [PRELIMINARY]:** not all 1639 building blocks sampled; the heights==0 half of ACViewer's `IsDungeon` and Z-vs-ground are unmeasured.

- **2026-05-31 (Portal openings: black-ceiling fix)** Root-caused the first room's opaque black "ceiling" (and the wood beams not showing): the extractor was emitting AC **portal** polygons as solid faces. A portal is a see-through opening into the adjacent cell, flagged `Stippling == NoPos`, and AC's renderer never draws it (confirmed against ACViewer `Render/R_CellStruct.cs` `Draw()`). Proven from the DAT with the new `dump-poly-stippling` (first room `0x860201AD` has two `NoPos` portals: poly 13 -> the wood-beam cell `0x...02E2` above, poly 12 -> the hallway `0x...01B4`). **Fixed `ExportEnvCell`** to skip `Stippling == NoPos` polygons (exact-equality, mirroring ACViewer). Safety proven academy-wide with the new `audit-portals`: across all **568** cells every skipped polygon is a declared portal (`NoPos ⊆ CellStruct.Portals`), **0** violations, **0** empty cells; 109 rendered portal sides correctly kept. Re-exported `0x860201AD` (26 -> 18 faces, the black `surf_2` gone), rebuilt `SM_860201AD` + reassigned the actor, **user-accepted the first-room render for this stage**. Decision recorded as ADR-0008. General fix (applies to every cell via `export-academy`); full academy re-import is a follow-up. (Open: same re-import for the other 567 cells.)
- **2026-05-31 (Step 3: furniture placed)** Fixed `DumpAcademyStatics` coordinate doubling (`stab.Frame` is landblock-absolute, same bug/fix as the lights); verified statics now sit inside their cells (median 4.1 m, 598/644 within 6 m). Re-exported setup meshes with the rebuilt acdat - **0/208 NaN/Inf** (the earlier NaN-bounds purge was the old buggy state, not the geometry). Imported: 200 setup meshes built, 644 actors placed, materials bound (1967 textured slots). First room renders furniture (rug, barrel, cabinets, wall pieces) over the textured shell. Runbook: `runbook/step3-furniture.md`. Open: per-object orientation/identity audit vs answer key; the floor placeholder diamonds are `import_npcs` markers (Step 4/5).
- **2026-05-31 (Steps 2 + 1: lighting decided, UV fix propagated)** **Step 2:** recorded ADR-0007 - indoor lighting = **Unlit, emissive=texture** (bright/even/deterministic, never black; matches the brightly-lit academy). The in-place "hybrid lit+emissive" wiring proved unreliable (rendered black); point-light warm accents are a documented deferred refinement (GI is off). Applied via `rebuild_academy_masters.py` (`AC_SHADING=unlit`) + `_refix_mi_textures.py`. **Step 1:** re-exported all 568 cells with the `PosUVIndices` fix (143s) and re-imported all meshes (`rebuilt=568`), so the corrected UVs + unlit materials are now academy-wide. First-room corner render confirms the shell (brown stone walls + masonry baseboard + blue tile floor) vs the answer key. **The map is now openable end-to-end** (all rooms textured + lit); furniture/NPCs still pending (Step 3).
